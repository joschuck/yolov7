...
def plot_images(
    ...
    scale_factor_h = min(shape[0] / h, 1.)

    h = math.ceil(scale_factor_h * h)
    ...
    for i, img in enumerate(images):
        ...
        if len(targets) > 0:
            ...
            labels = image_targets.shape[1] == nkpt * 2 + 6 if nkpt else image_targets.shape[1] == 6   # labels if no conf column
            ...
            if boxes.shape[1]:
                ...
                boxes[:, 1::2] *= scale_factor_h
            boxes[[0, 2]] += block_x
            boxes[[1, 3]] += block_y

            if nkpt and kpts.shape[1]:
                if kpts.max()<1.01:
                    kpts[list(range(0,len(kpts),steps))] *=w # scale to pixels
                    kpts[list(range(1,len(kpts),steps))] *= h
                kpts[list(range(0, len(kpts), steps))] *= scale_factor_w
                kpts[list(range(1, len(kpts), steps))] *= scale_factor_h
...
# TODO: this method needs to switch between keypoint and class format
...
def plot_skeleton_kpts(im, shape, kpts, steps, skeleton, conf: float = .3):
